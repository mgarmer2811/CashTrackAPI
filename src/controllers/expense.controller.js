...
function getISOWeekDateRange(yearInt, weekInt) {
  ...
  mondayOfTargetWeek.setUTCDate(mondayOfWeek1.getUTCDate() + (weekInt - 1) * 7);
...
exports.getAllExpenses = async (req, res, next) => {
  try {
    ...
    if (hasYear && !hasMonth && !hasWeek) {
      ...
      const lastDay = new Date(Date.UTC(yearInt, monthInt, 0)).getUTCDate();
      const mm = String(monthInt).padStart(2, "0");
      whereClause.created_at = {
        [Op.gte]: `${yearInt}-${mm}-01`,
        [Op.lte]: `${yearInt}-${mm}-${String(lastDay).padStart(2, "0")}`,
      };
    } else if (hasWeek) {
      if (!hasYear) {
        return res.status(400).json({
